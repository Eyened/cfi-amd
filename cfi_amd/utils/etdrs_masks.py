...
class ETDRS_masks:
    ...
    def calculate_area(self, binary_image):
        return float(binary_image.sum() * self.resolution ** 2)

    def calculate_count(self, binary_image):
        return int(np.max(measure.label(binary_image)))
    ...
    def create_svg(self, text_dict=None, crop=True, color='black'):
        ...
        if crop:
            ...
            viewbox = ' '.join(str(val * self.resolution)
                               for val in (-self.fovea_x, -self.fovea_y, self.w, self.h))
            width = self.w
            height = self.h
